...
const AtividadesTable = ({ data, searchTerm }) => {
  ...
  const handleSort = (key) => {
    let direction = 'asc';
    // Se já estiver ordenando por essa coluna e for ASC, inverte para DESC
    // Se for DESC, volta para 'asc' (ou poderia voltar para null se quisesse resetar)
  ...
  const processedData = useMemo(() => {
    ...
    if (sortConfig.key) {
      filtered.sort((a, b) => {
        ...
        // Tratamento especial para Status (lidar com nulos)
        if (sortConfig.key === 'status') {
          // Converte null/undefined para -1 para que fiquem sempre juntos na ordenação
    ...
    // Se não tiver sortConfig.key, ele mantém a ordem original do banco (Data DESC, Status ASC)
              ...
              <tr key={row.id} className="border-b border-[#f3f4f6] hover:bg-blue-50/30 transition-colors group">
                <td className="bg-[#fcfcfd] group-hover:bg-blue-50/30 py-1.5 px-2 border-r border-[#f3f4f6] text-center">
                  <div className="flex items-center justify-center gap-2">
                    <span className="font-bold tabular-nums tracking-tight">{row.data}</span>
                    <span className="text-gray-300">|</span>
                    <div className={`w-3 h-3 rounded-full shadow-sm ring-1 ring-white ${getStatusColor(row.status)}`}></div>
                  </div>
                </td>
                
                <td className="bg-[#fcfcfd] group-hover:bg-blue-50/30 py-1.5 px-2 border-r border-[#f3f4f6]">
                  <div className="flex flex-col items-center leading-tight">
                    <span className="font-bold text-[#0f172a]">{row.ativo}</span>
                    <span className="text-[10px] text-gray-500 text-center mt-0.5">{row.atividade}</span>
                  </div>
                </td>
                
                <td className="bg-[#fcfcfd] group-hover:bg-blue-50/30 py-1.5 px-2 border-r border-[#f3f4f6]">
                  <div className="flex justify-center gap-1.5 font-mono font-medium tabular-nums">
                    <span className="text-gray-600">{row.inicio.prog}</span>
                    <span className="text-gray-300">|</span>
                    <span className={`font-bold ${row.inicio.real === '--:--' ? 'text-gray-400' : 'text-gray-700'}`}> 
                      {row.inicio.real}
                    </span>
                  </div>
                </td>
                ...
                <td className="bg-[#fcfcfd] group-hover:bg-blue-50/30 py-1.5 px-2 border-r border-[#f3f4f6]">
                  <div className="flex justify-center gap-1.5 font-mono font-medium tabular-nums">
                    <span className="text-gray-600">{row.tempo.prog}</span>
                    <span className="text-gray-300">|</span>
                    <span className={`font-bold ${row.tempo.real === '--:--' ? 'text-gray-400' : 'text-gray-700'}`}>
                      {row.tempo.real}
                    </span>
                  </div>
                </td>
                ...
                <td className="bg-[#fcfcfd] group-hover:bg-blue-50/30 py-1.5 px-2 border-r border-[#f3f4f6]">
                  <div className="flex justify-center gap-1.5 text-[10px]">
                    <span className="text-gray-600">{row.local.prog}</span>
                    <span className="text-gray-300">|</span>
                    <span className="font-bold text-gray-700">{row.local.real}</span>
                  </div>
                </td>
                
                <td className="bg-[#fcfcfd] group-hover:bg-blue-50/30 py-1.5 px-2 border-r border-[#f3f4f6]">
                  <div className="flex justify-center gap-1.5 font-medium tabular-nums">
                    <span className="text-gray-600">{row.quant.prog}</span>
                    <span className="text-gray-300">|</span>
                    <span className="font-bold text-gray-700">{row.quant.real}</span>
                  </div>
                </td>
                
                <td className="bg-[#fffbf7] group-hover:bg-orange-50/30 py-1.5 px-3 text-left leading-snug">
                  {row.detalhe}
                </td>
              </tr>
            ))}
